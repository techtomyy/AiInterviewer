...
export default function Dashboard({ user }: DashboardProps) {
  ...
  const getConversionStatus = (session: any) => {
    ...
    if (!conversion && currentFileName.endsWith(".mp4")) {
      const rawFileName = currentFileName
        .replace("converted/", "raw/")
        .replace(".mp4", ".webm");
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          ...
          <p className="mb-4">
            Are you sure you want to delete the video from this interview
            session? The session will remain but the video will be permanently
